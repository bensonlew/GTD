...
** TODO 单物种背景图片不是黄色核查
** TODO 降低reactome的数据量 ？ 
** TODO 数据库上线？ uniprot reactome 基因组部署页面代码， 参数
...
** DONE smallRNA 问题 表达没有插入
   ...
   :END:      
** NEXT lncRNA stringtie 运行速度慢 部分内存超出
** small RNA 近源物种预测的 可以找到其在该基因组上的坐标么
** 有参注释kegg会超出， 转录本id
** TODO diamond 软件升级
** DONE 无参gsea反选
   CLOSED: [2020-12-09 Wed 08:26] SCHEDULED: <2020-12-08 Tue>
   :LOGBOOK:  
   - State "DONE"       from "NEXT"       [2020-12-09 Wed 08:26]
   :END:      
** DONE kegg png图片上传对象存储， 重新同步线上
   CLOSED: [2020-12-29 Tue 10:11] SCHEDULED: <2020-11-10 Tue>
   :LOGBOOK:  
   - State "DONE"       from "NEXT"       [2020-12-29 Tue 10:11]
   :END:      
** TODO wgcna dynamicTreeCut 初步研究
** TODO 基因组部署，添加物种上线
   SCHEDULED: <2020-12-16 Wed>
* 数据库升级
** DONE NR 分库
...
** emacs company 是否会有和 LSP complete 不兼容的现象， 目前python的补全任然靠anaconda
** 工作流参数默认值，新添加参数默认为旧版的参数， 旧版重运行不会有问题，但新版页面与流程可能不一致，除非把页面默认展示改为旧版，  默认为新版参数，旧流程又会出问题
* 其它
